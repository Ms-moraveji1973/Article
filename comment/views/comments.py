...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        #send email for authro , user and parent
        current_site = get_current_site(self.request)
        article = self.comment.content_object
        author_email = article.auther.email
        ...
        if author_email :
            email = EmailMessage(
                    "دیدگاه جدید",
                    "دید گاه جدیدی بر مقاله <{}> ک شما نویسنده آن هستید ارسال شد:\n{}{}"
                    .format(article,current_site,reverse("ArticleDetail",kwargs={'slug':article.slug})),
                    to=[author_email]
        )
            email.send() 
        ...
        if user_email :
            email = EmailMessage(
                    "دیدگاه شما ثبت شد",
                    "دیدگاه شما ثبت شد و به زودی به ان پاسخ میدهیم",
                    to=[user_email]
        )
            email.send() 
        ...
        if parent_email :
            email = EmailMessage(
                    "دیدگاه جدید",
                    "دید گاه جدیدی بر پاسخ شما در مقاله <{}>ارسال شد:\n{}{}"
                    .format(article,current_site,reverse("ArticleDetail",kwargs={'slug':article.slug})),
                    to=[parent_email]
        )
            email.send()
